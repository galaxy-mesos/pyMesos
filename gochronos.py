...
#vedi anche https://github.com/asher/chronos-python/blob/master/chronos/__init__.py
...
class GoChronosJobRunner(AsynchronousJobRunner):
    ...
    def __init__(self, app, nworkers, **kwargs):
        ...
        #self.chronos_cli = self.connect(self.runner_params["chronos_server"], self.runner_params["user"], self.runner_params["password"])
        ...
        #self.chronos_cli = self.connect(g_chronos_cli.servers,g_chronos_cli._user,g_chronos_cli._password)
        
        if not self.chronos_cli:
            log.debug("Connection failure!! Runner cannot be started")
        else:
            #log.debug("Chronos_cli CREATOOO!!!")
    ...
    def post_task(self, job_wrapper):
        ...
        try:
            ...
            if job_destination.params["gochrnos_docker_volumes"]:
             log.debug(job_destination.params["gochronos_docker_volumes"])
             volume = volume.split(",")
             for i in volume:
                temp = dict({"name": i})
                volumes.append(temp)
        ...
        """ try:
                if(job_destination.params["virtualenv"] == "true"):
                    log.debug("Virtual environment is set!!! DA AGGIUNGERE eventualmente")
        ...
        #gochronos_jobname="JOBTESTROX"
        #log.debug( "COMMAND IS: %s",job_wrapper.runner_command_line)   
        gochronos_job = {
              "name": gochronos_jobname,
              "command": "echo 'ROSANNA'",#job_wrapper.runner_command_line e diverso,
              "schedule":"R1//P10M", # inizia subito e ogni 10 minuti
              ...
              "executorFlags":"",
              "retries": 2,
              "async":False, #in galaxy pero' sono tutti asincroni???
              ...
              "uris": [], # ["file:///etc/docker.tar.gz"], #,"https://example.com/app/cool-script.sh",
       ...
       #devo seguire il template della class JobChronos in client.py????
       #one_of=["schedule"],
       #container_fields = [
        #"type",
        #"image"
...
#"cpus":0.1,"disk":256.0,"mem":128.0,"disabled":true,"softError":false,"dataProcessingJobType":false,"errorsSinceLastSuccess":0,"fetch":[],"uris":[],"environmentVariables":[],"arguments":[],"highPriority":false,"runAsUser":"root","constraints":[],"schedule":"R0/2016-11-16T09:51:17.000Z/PT24H","scheduleTimeZone":""}](.venv)galaxy@galaxy:~$ 
  ...
  ###############################################################################################
    def check_watched_item(self, job_state):
        ...
        if len(response) == 1:
            ...
            if succeeded:
                ...
                job_state.job_wrapper.change_state(model.Job.states.OK)
                self.create_log_file(job_state, job_chronos, "OK")
                self.mark_as_finished(job_state)
                return None
            elif failed:
                job_state.running = False
                job_state.job_wrapper.change_state(model.Job.states.ERROR)
                self.create_log_file(job_state, job_chronos,"ERROR")
    ...
    def stop_job(self, job):
        ...
        try:
            self.chronos_cli.delete(self.__produce_gochronos_job_name(job.job_wrapper.job_id)) 
